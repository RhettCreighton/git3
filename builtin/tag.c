...
static int build_tag_object(struct strbuf *buf, int sign, struct object_id *result, enum tag_type tag_type)
{
	/* For Git3, we need to mine the tag with proof-of-work */
	struct pow_data pow = {0};
	struct strbuf tag_content = STRBUF_INIT;
	char *object_line, *type_line, *tag_line, *tagger_line;
	char *message_start;
	...
	/* Parse the tag buffer to extract components */
	strbuf_addbuf(&tag_content, buf);
	
	/* Extract object line */
	object_line = strstr(tag_content.buf, "object ");
	if (!object_line)
		return error(_("malformed tag object"));
	object_line += 7; /* skip "object " */
	if (get_oid_hex(object_line, &object_oid) < 0)
		return error(_("invalid object ID in tag"));
	
	/* Extract type */
	type_line = strstr(tag_content.buf, "\ntype ");
	if (!type_line)
		return error(_("malformed tag object"));
	type_line += 6; /* skip "\ntype " */
	type_str = xstrndup(type_line, strcspn(type_line, "\n"));
	
	/* Extract tag name */
	tag_line = strstr(tag_content.buf, "\ntag ");
	if (!tag_line)
		return error(_("malformed tag object"));
	tag_line += 5; /* skip "\ntag " */
	tag_name = xstrndup(tag_line, strcspn(tag_line, "\n"));
	
	/* Extract tagger */
	tagger_line = strstr(tag_content.buf, "\ntagger ");
	if (tagger_line) {
		tagger_line += 8; /* skip "\ntagger " */
		tagger_str = xstrndup(tagger_line, strcspn(tagger_line, "\n"));
	}
	
	/* Find message start (after empty line) */
	message_start = strstr(tag_content.buf, "\n\n");
	if (message_start)
		message_start += 2;
	else
		message_start = "";
	...
	if (mine_pow_tag(&object_oid, type_str, tag_name, tagger_str, 
		...
		free(tag_name);
		free(tagger_str);
		strbuf_release(&tag_content);
		return error(_("failed to mine proof-of-work tag"));
	}
...
static void create_tag(const struct object_id *object, const char *object_ref,
		       ...
		       struct strvec *trailer_args, char *path, enum tag_type tag_type)
{
	enum object_type type;
	...
	if (build_tag_object(buf, opt->sign, result, tag_type) < 0) {
		if (path)
	    ...
	    struct repository *repo UNUSED)
{
	...
	enum tag_type tag_type = TAG_TYPE_NORMAL;
	int is_freeze = 0, is_clean = 0;
	...
	struct option options[] = {
		...
		OPT_GROUP(N_("Git3 tag types")),
		OPT_BOOL(0, "freeze", &is_freeze, N_("create a freeze tag")),
		OPT_BOOL(0, "clean", &is_clean, N_("create a clean tag")),
